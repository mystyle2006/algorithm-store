...
function solution(begin, target, words) {
  ...
  const changable = (word, _begin) => {
    return [...word].filter((w, i) => w !== _begin[i]).length === 1;
  }

  const dfs = (_begin, level, visited) => {
    if (visited.includes(target)) {
      answer = Math.min(level, answer);
    } else {
      const validWords = words.filter(_word => !visited.includes(_word) && changable(_word, _begin))
